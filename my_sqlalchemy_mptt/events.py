...
def _get_tree_table(mapper):
    for table in mapper.tables:
        if all(key in table.c for key in ['level', 'lft', 'rght', 'parent_id']):  # add rght
            return table
...
def mptt_before_insert(mapper, connection, instance):
    ...
    if instance.parent_id is None:
        ...
        connection.execute(
            table.update(
                and_(table.c.rght >= parent_pos_right, ############# add rght
                     table.c.tree_id == parent_tree_id)
            ).values(
                lft=case(
                    [
                ...
                rght=case( ############# add rght
                    [
                        (
                            table.c.rght >= parent_pos_right, ############# add rght
                            table.c.rght + 2 ############# add rght
                        )
                    ],
