...
async def get_items_responses(items_url_list, session):
    """Возвращает список response обьектов на все товары внутри категории"""
    items_response_list = []
    # try:
    print('get_items_responses')
    task_list = [asyncio.create_task(get_item_response(item_url, session)) for item_url in items_url_list[:1]]
    for future in asyncio.as_completed(task_list):
